...
def potrf(
    ...
    """Compute the Cholesky factor of a symmetric matrix using the multi-GPU potrf native kernel.

    Prepares inputs for the native ``potrf_mg`` kernel and executes it via
    ``jax.ffi.ffi_call`` under ``jax.jit`` and ``jax.shard_map``. Handles
    per-device padding driven by ``T_A`` and returns the lower-triangular
    Cholesky factor in the output buffer (upper triangle is undefined).
    ...
    Args:
        ...
        return_status (bool, optional): If True return ``(L, status)`` where
            ``status`` is a host-replicated int32 from the native solver. If
            False return ``L`` only. Default is False.
    ...
    Returns:
        Array or (Array, int): The Cholesky factor (row-sharded). If
...
def potrf_shardmap_ctx(a: Array, T_A: int, pad=True) -> Union[Array, Tuple[Array, int]]:
    """Compute a Cholesky factor for already-sharded inputs.

    This helper is a lower-level variant of :func:`potrf` intended for
    ...
    the native ``potrf_mg`` FFI target directly via ``jax.ffi.ffi_call``.
    ...
    Returns:
        tuple: ``(L, status)`` where ``L`` contains the lower-triangular Cholesky
            factor (same local/sharded layout as the input, with padding removed
            if applied) and ``status`` is the int32 status value returned by the
