...
#define SOLVER_DISPATCH_IMPL(impl, ...)            \
    ...
    default:                                       \
        ...
        ffi::Error PotrfMgImpl(int64_t N, int64_t batch_a,
        ...
        ffi::Error PotrfMgMpDispatch(gpuStream_t stream, ffi::ScratchAllocator scratch,
                                     ffi::AnyBuffer a, int64_t tile_size,
                                     ffi::Result<ffi::AnyBuffer> out, ffi::Result<ffi::Buffer<ffi::S32>> status)
        {
            ...
            {
                return ffi::Error::InvalidArgument(
                    "The input matrix a and output factor matrix of potrf must have the same element type");
            }
            FFI_RETURN_IF_ERROR(CheckShape(status->dimensions(), 1, "status", "potrf"));

            SOLVER_DISPATCH_IMPL(PotrfMgImpl, N, batch_a, stream, scratch, a, tile_size, out, status);

            return ffi::Error::InvalidArgument(absl::StrFormat(
                "Unsupported data type%s for potrf", absl::FormatStreamed(dataType)));
        }

        XLA_FFI_DEFINE_HANDLER_SYMBOL(PotrfMgMpFFI, PotrfMgMpDispatch,
                                      ffi::Ffi::Bind()
